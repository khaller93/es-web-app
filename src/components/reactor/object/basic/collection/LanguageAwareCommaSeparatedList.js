 ...
 */
const LanguageAwareCommaSeparatedList = ({role, mode, objects, getReactorConfigEntry, config: {show = 5}, intl}) => {
    const [more, setMore] = React.useState(false);

    const classes = useStyles();
    if (objects) {
        objects = objects.filter(obj => obj.type !== 'literal' || obj['xml:lang'] === intl.locale);
        if (objects.length === 1) {
            ...
            return <React.Fragment>
                ...
                {objectList.map((object, index) => {
                    return <RDFTermReactor key={index} role={role} mode={mode} term={object}
                                           getReactorConfigEntry={getConfig(object, getReactorConfigEntry)}
                                           intl={intl}/>;
...
export default LanguageAwareCommaSeparatedList;